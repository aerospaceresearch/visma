...
def test_inverse():
    ...
    if mat.isSquare():
        ...
        for i in mat.determinant():
            a += i.__str__()
        if a != "{0}":
            assert mat.inverse().__str__() == "[{-0.20952380952380953},{0.028571428571428605},{0.2761904761904762};{0.3047619047619048},{-0.3142857142857144},{-0.0380952380952381};{-0.009523809523809552},{0.22857142857142862},{-0.1238095238095238}]"
    ...
    if mat.isSquare():
        ...
        for i in mat.determinant():
            a += i.__str__()
        if a != "{0}":
            assert mat.inverse().__str__() == "[{-0.13043478260869565},{0.2173913043478261};{0.30434782608695654},{-0.17391304347826086}]"
